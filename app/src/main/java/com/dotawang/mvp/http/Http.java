...
public class Http {
    ...
    private Http() {
        ...
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create())   //设置返回类型为RXJava，或者回调
    ...
    private OkHttpClient getOkHttpClient() {
        if (sOkHttpClient == null) {
            synchronized (Http.class) {
                Cache cache = new Cache(new File(AppApplication.getInstance().getCacheDir(), "HttpCache"),
                        1024 * 1024 * 100);
                if (sOkHttpClient == null) {
                    sOkHttpClient = new OkHttpClient.Builder()
                            ...
                            .addInterceptor(KLoggingInterceptor).build();
                }
            }
        }
    ...
    private final Interceptor KLoggingInterceptor = new Interceptor() {
        @Override
        public Response intercept(Chain chain) throws IOException {
            Request request = chain.request();
